...
import { ctrlKey } from './defaultConfig'
...
export class Action {
  ...
  init() {
      ...
      if (this.graph.isEnabled() && !this.graph.isCellLocked(this.graph.getDefaultParent())) {
        ...
        try {
          ...
          if (cells != null) {
            ...
            if (bb != null) {
              const x = Math.round(this.graph.snap(this.graph.popupMenuHandler.triggerX / s - dx))
              const y = Math.round(this.graph.snap(this.graph.popupMenuHandler.triggerY / s - dy))
    ...
    //删除
    this.put('delete', new ActionItem(() => {
      const cells = this.graph.getSelectionCells()
      this.graph.deleteCells(cells)
    }, '删除', 'Delete'))
    //撤销
    this.put('undo', new ActionItem(() => {
      this.undoManager.undo()
    }, '撤销', `${ctrlKey}+Z`))
    //取消撤销
    this.put('redo', new ActionItem(() => {
      this.undoManager.redo()
    }, '取消撤销', `${ctrlKey}+Y`))
    //置于底部
    this.put('toBack', new ActionItem(() => {
      this.graph.orderCells(false)
    }, '置于底部', `${ctrlKey}+Shift+B`))
    //置于顶部
    this.put('toFront', new ActionItem(() => {
      this.graph.orderCells(true)
    }, '置于顶部', `${ctrlKey}+Shift+Y`))
    //组合
    this.put('group', new ActionItem(() => {
    ...
    }, '取消组合', `${ctrlKey}+Shift+U`))
    //选中所有edges
    this.put('selectEdges', new ActionItem(() => {
      this.graph.selectEdges(this.graph.getDefaultParent())
    }, '选中所有边', `${ctrlKey}+Shift+I`))
    //选中所有vertex
    this.put('selectVertices', new ActionItem(() => {
      this.graph.selectVertices(this.graph.getDefaultParent(), true)
    }, '选中所有图元', `${ctrlKey}+Shift+V`))
    this.put('zoomIn', new ActionItem(() => {
      this.graph.zoomIn()
    }, '放大', `${ctrlKey}+加`))
    this.put('zoomOut', new ActionItem(() => {
      this.graph.zoomOut()
    }, '缩小', `${ctrlKey}+-`))
    this.put('addChildren', new ActionItem(() => {
      if (this.graph.isEnabled()) {
        this.graph.getModel().beginUpdate()
        try {
