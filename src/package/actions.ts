            ...
            try {
              ...
              if (cells != null) {
                ...
                for (let i = 0; i < cells.length && includeEdges; i++) {
                  includeEdges =
                    includeEdges && this.graph.model.isEdge(cells[i])
                }
                ...
                bb =
                  bb != null
                    ? bb
                    : this.graph.getBoundingBoxFromGeometry(cells, includeEdges)

                if (bb != null) {
                  const x = Math.round(
                    this.graph.snap(
                      this.graph.popupMenuHandler.triggerX / s - dx
                    )
                  )
                  const y = Math.round(
                    this.graph.snap(
                      this.graph.popupMenuHandler.triggerY / s - dy
                    )
                  )
          ...
          if (this.graph.isEnabled()) {
            ...
            if (cells.length > 1) {
              ...
              this.graph.setSelectionCell(
                this.graph.groupCells(group, 0, cells)
              )
            }
            // }
          }
